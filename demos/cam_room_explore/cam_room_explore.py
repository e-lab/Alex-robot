...
class CameraOverlayManager:
  ...
  def update(self, viewer: mujoco.viewer.Handle, *, force: bool = False) -> None:
    ...
    width = max(1, int(viewport.width * OVERLAY_WIDTH_FRACTION))
    ...
    depth_resized = cv2.cvtColor(depth_resized, cv2.COLOR_BGR2RGB)
    viewer.set_images([(rect_rgb, rgb_resized), (rect_depth, depth_resized)])
...
def run_manual(
  ...
  *,
  ...
  overlay_manager = CameraOverlayManager(controller, detector=detector)
