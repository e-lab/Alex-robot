"""Convert LAFAN retargeted CSV motions to Alex tracking npz format.
...
# Alex actuated joints in model order used by tracking action.
ALEX_JOINT_ORDER = (
  "spine_z",
  "left_hip_x",
  "left_hip_z",
...
def _load_csv(path: Path, line_range: tuple[int, int] | None) -> np.ndarray:
  ...
  if start < 1 or end < start:
    raise ValueError("line_range must be 1-indexed and satisfy start <= end.")
...
def _remap_g1_csv_to_alex(csv_data: np.ndarray) -> np.ndarray:
  if csv_data.ndim != 2:
  ...
  root = csv_data[:, :7]
  g1_joints = csv_data[:, 7 : 7 + len(G1_JOINT_ORDER)]
  g1_index = {name: i for i, name in enumerate(G1_JOINT_ORDER)}

  alex_joint_cols = []
  for alex_joint in ALEX_JOINT_ORDER:
    src = ALEX_FROM_G1[alex_joint]
    alex_joint_cols.append(g1_joints[:, g1_index[src]])

  alex_joints = np.stack(alex_joint_cols, axis=1)
  return np.concatenate([root, alex_joints], axis=1)


def convert_lafan_csv_to_alex_npz(
  input_csv: Path,
  output_npz: Path,
  input_fps: float,
  output_fps: float,
  ...
  csv_data = _load_csv(input_csv, line_range)
  alex_motion_xyzw = _remap_g1_csv_to_alex(csv_data)
  ...
  resampled = MotionResampler(
    motion_xyzw=torch.from_numpy(alex_motion_xyzw),
    input_fps=input_fps,
...
def main() -> None:
  parser = argparse.ArgumentParser(
    description="Convert LAFAN/G1-order CSV motion to Alex tracking npz."
  )
  parser.add_argument(
    "--input_file",
    "--input-csv",
  ...
  parser.add_argument(
    ...
    help="Directory scanned when --input-csv is not provided.",
  )
  parser.add_argument(
    "--output_file",
    "--output-npz",
  ...
  parser.add_argument(
    ...
    default=None,
    help=(
      "Optional 1-indexed inclusive frame range: START END. "
      "Equivalent to the reference csv_to_npz script."
    ),
  )
  ...
  if args.output_npz is not None:
    ...
    output_npz = args.motions_dir / f"{input_csv.stem}_alex_tracking.npz"
  line_range = tuple(args.frame_range) if args.frame_range is not None else None

  convert_lafan_csv_to_alex_npz(
    input_csv=input_csv,
